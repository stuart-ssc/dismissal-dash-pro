...
-- Add is_reviewed column to classes table
ALTER TABLE public.classes ADD COLUMN is_reviewed boolean NOT NULL DEFAULT false;

-- Update convert_classes_to_groups to accept reviewed class IDs
CREATE OR REPLACE FUNCTION public.convert_classes_to_groups(
  p_school_id bigint,
  p_session_id uuid,
  p_conversions jsonb,
  p_reviewed_class_ids uuid[] DEFAULT '{}'::uuid[]
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
