...
BEGIN
  ...
  v_completeness := (
    CASE WHEN v_student_metrics.total > 0 THEN
      (1 - (v_student_metrics.missing_contact::NUMERIC / v_student_metrics.total * 0.15) 
         - (v_student_metrics.missing_parent::NUMERIC / v_student_metrics.total * 0.15)
         - (v_student_metrics.missing_ic_id::NUMERIC / v_student_metrics.total * 0.35)
         - (v_student_metrics.without_classes::NUMERIC / v_student_metrics.total * 0.35)
      ) * 50
    ELSE 0 END
  ) + (
