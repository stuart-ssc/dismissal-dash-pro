...
-- 1. Change FK on special_use_group_managers.manager_id from profiles to teachers
ALTER TABLE public.special_use_group_managers
  DROP CONSTRAINT IF EXISTS special_use_group_managers_manager_id_fkey;

ALTER TABLE public.special_use_group_managers
  ADD CONSTRAINT special_use_group_managers_manager_id_fkey
  FOREIGN KEY (manager_id) REFERENCES public.teachers(id) ON DELETE CASCADE;

-- 2. Update convert_classes_to_groups to carry over teachers as managers
...
BEGIN
  ...
  LOOP
    ...
    IF v_action = 'convert' THEN
      ...
      -- Copy students from class_rosters
      ...
      -- Copy teachers as group managers
      INSERT INTO special_use_group_managers (group_id, manager_id, assigned_by)
      SELECT v_new_group_id, ct.teacher_id, auth.uid()
      FROM class_teachers ct
      WHERE ct.class_id = v_class_id
      ON CONFLICT DO NOTHING;

      GET DIAGNOSTICS v_manager_count = ROW_COUNT;
      v_total_managers := v_total_managers + v_manager_count;
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'total_managers_assigned', v_total_managers,
