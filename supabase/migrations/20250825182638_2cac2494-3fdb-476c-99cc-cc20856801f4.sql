-- Add new columns to dismissal_runs for schedule-driven system
ALTER TABLE public.dismissal_runs 
ADD COLUMN scheduled_start_time timestamp with time zone,
ADD COLUMN preparation_start_time timestamp with time zone,
ADD COLUMN bus_completed boolean DEFAULT false,
ADD COLUMN car_line_completed boolean DEFAULT false,
ADD COLUMN walker_completed boolean DEFAULT false,
ADD COLUMN bus_completed_at timestamp with time zone,
ADD COLUMN car_line_completed_at timestamp with time zone,
ADD COLUMN walker_completed_at timestamp with time zone,
ADD COLUMN bus_completed_by uuid,
ADD COLUMN car_line_completed_by uuid,
ADD COLUMN walker_completed_by uuid;

-- Update status column to include new states
ALTER TABLE public.dismissal_runs 
DROP CONSTRAINT IF EXISTS dismissal_runs_status_check;

ALTER TABLE public.dismissal_runs 
ADD CONSTRAINT dismissal_runs_status_check 
CHECK (status IN ('scheduled', 'preparation', 'active', 'completed'));

-- Create function to calculate dismissal times from plan
...
-- Create function to auto-update dismissal run status based on time
...
CREATE TRIGGER update_dismissal_run_status_trigger
  BEFORE UPDATE ON public.dismissal_runs
  FOR EACH ROW
  EXECUTE FUNCTION public.update_dismissal_run_status();

-- Function to create scheduled dismissal runs
