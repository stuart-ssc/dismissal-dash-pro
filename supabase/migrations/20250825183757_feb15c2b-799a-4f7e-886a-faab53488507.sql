-- Fix the dismissal run creation for automated scheduling
-- Make started_by nullable since scheduled runs are created automatically
ALTER TABLE public.dismissal_runs ALTER COLUMN started_by DROP NOT NULL;

-- Update the function to handle automated creation properly
...
-- Now reset today's dismissal run data
DELETE FROM public.dismissal_runs WHERE date = CURRENT_DATE;

-- Create a new properly scheduled dismissal run
