...
export default function ClassroomMode() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const {
        data,
        error
      } = await supabase.from("dismissal_plans").select("name").eq("id", planId).single();
  ...
  useEffect(() => {
    ...
    (async () => {
      const {
        data: ct,
        error: ctErr
      } = await supabase.from("class_teachers").select("class_id").eq("teacher_id", user.id).order("assigned_at", {
        ascending: true
      }).limit(1).maybeSingle();
      ...
      const {
        data: cls,
        error: clsErr
      } = await supabase.from("classes").select("class_name").eq("id", ct.class_id).maybeSingle();
      if (clsErr) {
  ...
  const fetchActiveGroups = useMemo(() => async () => {
    if (!runId) return;
    if (!planId) {
      setGroups([]);
    ...
    const {
      data: drg,
      error: drgErr
    } = await supabase.from("dismissal_run_groups").select("id,dismissal_group_id,activated_at").eq("dismissal_run_id", runId).is("deactivated_at", null).order("activated_at", {
      ascending: true
    });
    if (drgErr) {
    ...
    const groupsInfo = await Promise.all(groupIds.map(async gid => {
      const {
        data,
        error
      } = await supabase.from("dismissal_groups").select("id,name,group_type,dismissal_plan_id").eq("id", gid).single();
      if (error) {
        console.error(error);
        return null;
      }
    ...
    for (const rg of drg || []) {
      ...
      if (groupType && groupType.toLowerCase().includes("bus")) {
        const {
          data: gb
        } = await supabase.from("dismissal_group_buses").select("bus_id").eq("dismissal_group_id", rg.dismissal_group_id);
        const busIds = (gb || []).map(x => x.bus_id);
        if (busIds.length > 0) {
          // Only include buses that have been checked in (present) and not departed for this run
          const {
            data: presentEvents
          } = await supabase.from("bus_run_events").select("bus_id, check_in_time, departed_at").eq("dismissal_run_id", runId!).in("bus_id", busIds).not("check_in_time", "is", null).is("departed_at", null);
          const presentBusIds = (presentEvents || []).map(e => e.bus_id);
          if (presentBusIds.length > 0) {
            const {
              data: busList
            } = await supabase.from("buses").select("id,bus_number").in("id", presentBusIds).eq("school_id", schoolId ?? -1);
            buses = (busList || []).map(b => ({
              id: b.id,
  ...
  useEffect(() => {
    if (!runId) return;
    const channel = supabase.channel("dismissal-run-groups-stream").on("postgres_changes", {
      event: "*",
      schema: "public",
  ...
  return <div className="min-h-screen w-full bg-background text-foreground p-6">
      <div className="mx-auto max-w-6xl">
        ...
        {run && !planId && <Alert className="mb-6">
        ...
        {isLoading || loadingGroups ? <div className="flex items-center gap-2 text-muted-foreground">
            <Loader2 className="animate-spin" />
            Loading...
          </div> : groups.length === 0 ? <p className="text-xl text-muted-foreground">No groups are active yet.</p> : <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {groups.map(g => <Card key={g.id} className="border-2">
                <CardHeader>
                ...
                <CardContent>
                  {g.group_type && g.group_type.toLowerCase().includes("bus") ? <div>
                      <p className="text-muted-foreground mb-2">Buses:</p>
                      <div className="flex flex-wrap gap-2">
                        {g.buses.map(b => <span key={b.id} className="px-3 py-1 rounded-full bg-secondary text-secondary-foreground text-lg">
                            {b.bus_number}
                          </span>)}
                      </div>
                    </div> : <p className="text-muted-foreground">This group is now dismissed.</p>}
                </CardContent>
              </Card>)}
          </div>}
      </div>
